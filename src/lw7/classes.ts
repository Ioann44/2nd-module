...
import { OwnerI, VehicleI, CarI, MotorbikeI, VehicleStorageI } from "./interfaces";
...
Номер: ${this.docNumber}\n`);
	...
	export class Car implements CarI {
		...
		getBodyClassInfo(): { body: string; class: string; } {
			return { body: this.carBody, class: this.carClass };
		}
	}
	...
	export class BikeStorage implements VehicleStorageI<MotorbikeI> {
		...
		// task 1
		getSortedBackwardByModels(): MotorbikeI[] {
			let storageCopy = [...this.storage];
			storageCopy.sort((a: MotorbikeI, b: MotorbikeI) => {
				if (a.model < b.model) {
					return 1;
		...
		// task 2
		getAllByCompany(company: string): MotorbikeI[] {
			const result: MotorbikeI[] = this.storage.filter(
				moto => moto.company == company
			);
			return result;
		}
	}
